...
class ODIRYOLOTrainer:
    ...
    def load_saved_model(self, model_path, metadata_path=None):
        ...
        if metadata_path.exists():
            ...
            if self.training_history:
                print(f"Loaded training history with {len(self.training_history)} epochs")
    ...
    def train_from_folder(self, data_folder, run_name=None, epochs=100, imgsz=512, 
        ...
        train_args = {
            'data': str(data_folder),  # Ultralytics expects a folder with train/val subfolders
            'epochs': epochs,
            'imgsz': imgsz,
            'batch': batch_size,
            'patience': patience,
        ...
        # Evaluate on validation set and plot confusion matrix
        print("\nEvaluating on validation set and generating confusion matrix...")
        val_metrics = self.evaluate_on_validation(data_folder, imgsz)
    ...
    def plot_training_metrics(self, run_name):
        ...
        # Plot 2: Accuracy
        axes[0, 1].plot(epochs, accuracies, 'g-', label='Accuracy', linewidth=2)
        ...
        for f1 in f1_scores_levels:
            ...
            r = np.clip(r, 0, 1)
            valid = p > f1/2
            axes[1, 0].plot(p[valid], r[valid], '--', alpha=0.5, label=f'F1={f1}')
        ...
        if len(train_losses) > 0 and len(val_losses) > 0:
            ...
            axes[1, 1].plot(epochs, loss_diff, 'c-', label='|Train Loss - Val Loss|', linewidth=2)
        ...
        self._save_individual_plots(plots_dir, epochs, train_losses, val_losses, accuracies, f1_scores)
    ...
    def _save_individual_plots(self, plots_dir, epochs, train_losses, val_losses, accuracies, f1_scores):
        ...
        plt.plot(epochs, val_losses, 'r-', linewidth=3, label='Validation Loss')
        plt.xlabel('Epoch', fontsize=12)
    ...
    def _log_training_curves_to_wandb(self):
        """Log training curves to W&B"""
        if not self.training_history or not self.use_wandb or not self.wandb_run:
            return
        ...
        for f1 in f1_scores_levels:
            ...
            r = np.clip(r, 0, 1)
            valid = p > f1/2
            axes[1, 1].plot(p[valid], r[valid], '--', alpha=0.5, label=f'F1={f1}')
        ...
        # Log to W&B
        self.wandb_run.log({"training_curves": wandb.Image(fig)})
        ...
        for img_path in tqdm(image_files, desc=f"Evaluating {split_name}"):
            ...
            if true_label in self.class_names:
                ...
                try:
                    ...
                    print(f"Warning: Could not parse label '{true_label}' for image {img_path.name}")
                    continue
    ...
    def plot_confusion_matrix(self, cm, split_name='validation', imgsz=512):
        ...
        ax1.set_title(f'Confusion Matrix - {split_name.capitalize()} Set\n(Absolute Values)', fontsize=14)
        ...
        ax2.set_title(f'Confusion Matrix - {split_name.capitalize()} Set\n(Normalized)', fontsize=14)
        ...
        self._save_simple_confusion_matrix(cm, split_name, imgsz, plots_dir, timestamp)
    ...
    def _display_prediction_result(self, img_path, pred_class, confidence, top_predictions, pred_probs):
        ...
        colors = ['red' if i == np.argmax(pred_probs) else 'skyblue' for i in range(len(self.class_names))]
    ...
    def run_inference_benchmark(self, data_folder, num_iterations=100, batch_sizes=[1, 4, 8, 16], imgsz=512):
        ...
        # Find a sample image
        image_files = list(test_folder.glob('*/*.jpg')) or list(test_folder.glob('*.jpg'))
    ...
    def save_evaluation_results(self, metrics_dict, benchmark_results=None, split_name='test'):
        ...
        metrics_path = results_dir / f'{split_name}_metrics_{self.model_version}_{timestamp}.csv'
        ...
        if f1_per_class:
            ...
            f1_path = results_dir / f'{split_name}_f1_scores_{self.model_version}_{timestamp}.csv'
        ...
        if benchmark_results:
            ...
            bench_path = results_dir / f'{split_name}_benchmark_{self.model_version}_{timestamp}.csv'
...
def example_usage():
    """Example of how to use the trainer in Jupyter Notebook"""
    
    # Option 1: Without W&B
    print("=== Training without W&B ===")
    trainer = ODIRYOLOTrainer(model_version='yolo11n-cls', use_wandb=False)
    results = trainer.train_from_folder(
        data_folder='./datasets/odir',
        run_name='yolo11n_512_test',
        epochs=50,
        imgsz=512,
        batch_size=16
    )
    ...
    test_metrics = trainer.evaluate_on_test('./datasets/odir', imgsz=512)
    trainer.save_evaluation_results(test_metrics, split_name='test')
    ...
    # Option 2: With W&B
    print("\n=== Training with W&B ===")
    # Initialize W&B outside the trainer
    wandb.init(project="odir-yolo-classification", name="yolo11s_512_experiment")
    
    trainer_wandb = ODIRYOLOTrainer(model_version='yolo11s-cls', use_wandb=True, wandb_run=wandb)
    results_wandb = trainer_wandb.train_from_folder(
        data_folder='./datasets/odir',
        run_name='yolo11s_512_wandb',
        epochs=50,
        imgsz=512,
        batch_size=16
    )
    ...
    # Evaluate and save
    test_metrics_wandb = trainer_wandb.evaluate_on_test('./datasets/odir', imgsz=512)
    trainer_wandb.save_evaluation_results(test_metrics_wandb, split_name='test')
    trainer_wandb.save_model()
    
    # Finish W&B run
    trainer_wandb.finish()
    
    return trainer, trainer_wandb


if __name__ == "__main__":
    # This can be run directly or imported in Jupyter
    print("ODIR YOLO Classification Trainer")
    print("To use in Jupyter: from odir_yolo_trainer import ODIRYOLOTrainer")