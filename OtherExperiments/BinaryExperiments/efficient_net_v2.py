...
class ODIRFolderDataset(Dataset):
    ...
    def __init__(self, root_dir, split='train', transform=None, image_size=224, 
        ...
        Args:
            ...
            binary_class: If provided, binary classification for this specific class
                         Format: 'class_name' or None for multi-class
        """
        ...
        if binary_class is not None:
            if binary_class not in self.class_names:
                raise ValueError(f"Invalid binary class '{binary_class}'. Must be one of: {self.class_names}")
            self.is_binary = True
            self.binary_class_idx = self.class_names.index(binary_class)
            # For binary classification, we use class names: [negative, positive]
            self.binary_class_names = [f'not_{binary_class}', binary_class]
            self.num_classes = 2
            print(f"⚠️  Binary classification mode: {binary_class} vs all other classes")
        else:
            self.is_binary = False
            self.binary_class_idx = None
            self.binary_class_names = None
        ...
        for class_name in self.class_names:
            ...
            # For binary classification: remap labels
            if self.is_binary:
                if class_name == binary_class:
                    binary_label = 1  # Positive class
                else:
                    binary_label = 0  # Negative class
                label = binary_label
            else:
                label = class_idx
    ...
    def _print_class_distribution(self):
        ...
        if self.is_binary:
            ...
            print(f"\nBinary class distribution in {self.split} split:")
            print(f"  {self.binary_class_names[1]}: {pos_count} samples ({pos_count/len(self.samples)*100:.1f}%)")
            print(f"  {self.binary_class_names[0]}: {neg_count} samples ({neg_count/len(self.samples)*100:.1f}%)")
        else:
...
class EfficientNetTrainer:
    ...
    def __init__(self, config):
        ...
        self.binary_class = config.get('binary_class', None)
        self.is_binary = self.binary_class is not None
        self.num_classes = 2 if self.is_binary else config.get('num_classes', 8)
        
        print(f"\n{'='*50}")
        if self.is_binary:
            print(f"🚀 BINARY CLASSIFICATION MODE: {self.binary_class} vs all other classes")
        else:
            print(f"🚀 MULTI-CLASS CLASSIFICATION MODE: {self.num_classes} classes")
        print(f"{'='*50}")
        ...
        if self.is_binary:
            ...
            if pos_weight is not None:
                ...
                print(f"Using weighted BCE loss with pos_weight: {pos_weight_value:.4f}")
                self.criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight.to(self.device))
            else:
                print("Using BCE loss without weighting")
                self.criterion = nn.BCEWithLogitsLoss()
        else:
            # Multi-class classification with label smoothing
    ...
    def _calculate_pos_weight(self):
        ...
        if not self.is_binary or self.train_dataset is None:
            return None
        
        # Count positive and negative samples
        neg_count = sum(1 for _, label in self.train_dataset.samples if label == 0)
        pos_count = len(self.train_dataset.samples) - neg_count
        
        if pos_count == 0 or neg_count == 0:
            print(f"Warning: One of the classes has 0 samples. Positive: {pos_count}, Negative: {neg_count}")
            return None
        ...
        print(f"Positive weight for BCE loss: {pos_weight.item():.4f}")
    ...
    def _create_model(self):
        ...
        print(f"Loading {model_name} with {self.num_classes} classes...")
        
        # Load pretrained EfficientNet
        model = EfficientNet.from_pretrained(model_name, num_classes=self.num_classes)
        
        # For binary classification, we need to modify the output layer
        if self.is_binary:
            # Keep 2 outputs but we'll use only the positive class logit
            # Or we can change to 1 output
            # Option 1: Keep 2 outputs and use softmax
            # Option 2: Change to 1 output and use sigmoid
            # Let's keep 2 outputs for consistency with multi-class
            pass
    ...
    def _create_datasets(self):
        ...
        if self.is_binary:
            self.class_names = self.train_dataset.binary_class_names
        else:
    ...
    def _create_dataloaders(self):
        ...
        if self.is_binary:
            print(f"   Mode:          Binary ({self.binary_class} vs all)")
        else:
            print(f"   Mode:          Multi-class ({self.num_classes} classes)")
        ...
        for batch_idx, (images, targets, _) in enumerate(pbar):
            ...
            if self.config.get('use_amp', False) and torch.cuda.is_available():
                with torch.cuda.amp.autocast():
                    ...
                    if self.is_binary:
                        # For binary classification with 2 outputs, we need to use CrossEntropyLoss
                        # Or we can extract the positive class logit
                        targets = targets.long()  # Convert to long for CrossEntropyLoss
    ...
    def plot_confusion_matrix(self, y_true, y_pred, class_names, split='val', save_path=None):
        ...
        plt.figure(figsize=(10, 8) if len(class_names) <= 8 else (12, 10))
        ...
        plt.xticks(rotation=45 if len(class_names) > 4 else 0, ha='right' if len(class_names) > 4 else 'center')
        plt.yticks(rotation=0)
        
        # Add percentage annotations for binary or small number of classes
    ...
    def plot_training_history(self, save_path=None):
        ...
        plt.suptitle(f'Training History - {"Binary" if self.is_binary else "Multi-class"} Classification', 
                    fontsize=16, y=1.02)
        plt.tight_layout()
    ...
    def load_model(self, path):
        ...
        # Update binary classification mode
        if 'is_binary' in checkpoint:
            self.is_binary = checkpoint['is_binary']
            self.binary_class = checkpoint.get('binary_class', None)
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            # Validate
            val_loss, val_acc, val_f1, val_preds, val_targets, val_probs, val_inf_time = self.validate()
            ...
            if self.scheduler and not isinstance(self.scheduler, optim.lr_scheduler.OneCycleLR):
                if isinstance(self.scheduler, ReduceLROnPlateau):
                    # Use F1 score for both binary and multi-class
    ...
    def _create_results_table(self, y_true, y_pred, y_probs):
        ...
        if self.is_binary:
            ...
            wandb.log({"binary_classification_metrics": results_table})
        else:
    ...
    def predict_single_image(self, image_path):
        ...
        with torch.no_grad():
            ...
            if self.is_binary:
                ...
                top3_probs, top3_indices = torch.topk(probabilities[0], min(3, len(self.class_names)))
                top2_predictions = [
        ...
        return {
            ...
            'top_predictions': top2_predictions,
